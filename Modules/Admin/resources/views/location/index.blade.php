      ...
      <div class="card mt-3">
        <div class="card-body">
          ...
          <ul class="nav nav-tabs" id="locationTabs" role="tablist">
            <li class="nav-item">
              <a class="nav-link active" id="countries-tab" data-toggle="tab" href="#countries" role="tab">
                Countries
              </a>
            </li>
            <li class="nav-item">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="text-center">
                        <a href="{{ route('admin.add.update.country', base64_encode($country->id)) }}" class="text-warning" title="Edit">
                          <i class="fas fa-edit"></i>
                        </a>
                        <a href="javascript:void(0)" class="text-danger delete-country" data-id="{{ base64_encode($country->id) }}" title="Delete">
                ...
                <table class="table table-striped align-middle commontable" id="regionsTable">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>{{ $region->created_at ? $region->created_at->format('d M Y') : '—' }}</td>
                      <td class="text-center">
                        <a href="{{ route('admin.add.update.region', base64_encode($region->id)) }}" class="text-warning" title="Edit">
                          <i class="fas fa-edit"></i>
                        </a>
                        <a href="javascript:void(0)" class="text-danger delete-region" data-id="{{ base64_encode($region->id) }}" title="Delete">
                          <i class="fas fa-trash"></i>
                        </a>
                      </td>
                    </tr>
                    @empty
                    <tr>
                      <td colspan="5" class="text-center text-muted py-4">No regions found.</td>
                    </tr>
                    @endforelse
                  </tbody>
                </table>
                ...
                <table class="table table-striped align-middle commontable" id="citiesTable">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>{{ optional(optional($city->region)->country)->country ?? '—' }}</td>
                      <td>{{ $city->created_at ? $city->created_at->format('d M Y') : '—' }}</td>
                      <td class="text-center">
                        <a href="{{ route('admin.add.update.city', base64_encode($city->id)) }}" class="text-warning" title="Edit">
                          <i class="fas fa-edit"></i>
                        </a>
                        <a href="javascript:void(0)" class="text-danger delete-city" data-id="{{ base64_encode($city->id) }}" title="Delete">
                          <i class="fas fa-trash"></i>
                        </a>
                      </td>
                    </tr>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.delete-country', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('admin.country.delete', ':id') }}".replace(':id', id),
                    type: "DELETE",
                    data: { _token: "{{ csrf_token() }}" },
                    success: function (response) {
                        if (response.status) {
                            Swal.fire('Deleted!', response.message, 'success');
                            $('.delete-country[data-id="' + id + '"]').closest('tr').fadeOut();
                        } else {
    ...
    $(document).on('click', '.delete-region', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('admin.region.delete', ':id') }}".replace(':id', id),
                    type: "DELETE",
                    data: { _token: "{{ csrf_token() }}" },
                    success: function (response) {
                        if (response.status) {
                            Swal.fire('Deleted!', response.message, 'success');
                            $('.delete-region[data-id="' + id + '"]').closest('tr').fadeOut();
                        } else {
                            Swal.fire('Error!', response.message, 'error');
                        }
                    },
    ...
    $(document).on('click', '.delete-city', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('admin.city.delete', ':id') }}".replace(':id', id),
                    type: "DELETE",
                    data: { _token: "{{ csrf_token() }}" },
                    success: function (response) {
                        if (response.status) {
                            Swal.fire('Deleted!', response.message, 'success');
                            $('.delete-city[data-id="' + id + '"]').closest('tr').fadeOut();
                        } else {
                            Swal.fire('Error!', response.message, 'error');
                        }
                    },
