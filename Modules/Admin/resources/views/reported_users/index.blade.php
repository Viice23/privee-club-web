   ...
   <div class="content-header">
      <div class="container-fluid">
         <div class="page-header">
            <h3><i class="las la-user-plus"></i> Reported Users</h3>
         </div>
      </div>
   </div>
                        ...
                        <tbody>
                            @foreach($report as $key => $val)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>
                                        {{ optional($val->reported_by)->first_name }} {{ optional($val->reported_by)->last_name }}
                                    </td>
                                    <td>
                                        {{ optional($val->reported_to)->first_name }} {{ optional($val->reported_to)->last_name }}
                                    </td>
                                    <td>{{ optional($val->reason)->report_reason ?? '' }}</td>
                                    <td>{{ $val->created_at ? $val->created_at->format('d M Y H:i:s') : '' }}</td>
                                    <td>{{ $val->message ?? '' }}</td>
                                    <td>
...
@section('script')
    <script>
        $(document).on('change', '.change-status', function () {
            ...
            $.ajax({
                url: "{{ route('admin.report.update.status') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id,
