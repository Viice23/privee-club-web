...
.action-icons i.fa-edit { color: #ffc107; }
.action-icons i.fa-trash { color: #dc3545; }
.action-icons i.fa-toggle-on { color: #28a745; }
.action-icons i.fa-toggle-off { color: #6c757d; }
  ...
  <section class="content">
    <div class="container-fluid">
      <div class="card mt-3">
        <div class="card-body">
          <!-- Global Toggle -->
          <div class="d-flex justify-content-between align-items-center mb-4 p-3" style="background: #f8f9fa; border-radius: 8px;">
            <div>
              <label class="font-weight-bold mb-0">Show Fake Users on Mobile App</label>
              <p class="text-muted mb-0" style="font-size: 12px;">Toggle to show/hide all fake users in mobile API</p>
            </div>
            <label class="switch">
          ...
          <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center gap-2">
              ...
              <button type="button" class="btn btn-secondary" id="applyBulkAction" disabled>Apply</button>
            </div>
          </div>
          ...
          <div class="table-responsive">
            <table class="table table-striped align-middle commontable" id="readyMembersTable">
              ...
              <tbody>
                ...
                <tr>
                  <td>
                    <input type="checkbox" class="member-checkbox" value="{{ base64_encode($member->id) }}">
                  </td>
                  <td>{{ $index + 1 }}</td>
                  ...
                  <td class="text-center action-icons">
                    <a href="{{ route('admin.view.ready.member', base64_encode($member->id)) }}" title="View">
                      <i class="fa fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.edit.ready.member', base64_encode($member->id)) }}" title="Edit">
                      <i class="fa fa-edit"></i>
                    </a>
                    <a href="javascript:void(0)" class="toggle-status" data-id="{{ base64_encode($member->id) }}" title="Toggle Status">
                      <i class="fa {{ $member->is_active == 1 ? 'fa-toggle-on' : 'fa-toggle-off' }}"></i>
                    </a>
                    <a href="javascript:void(0)" class="delete-member" data-id="{{ base64_encode($member->id) }}" title="Delete">
                ...
                @empty
                <tr>
                  <td colspan="10" class="text-center text-muted py-4">No ready members found.</td>
                </tr>
                @endforelse
              </tbody>
            </table>
...
$(document).ready(function () {
    ...
    $('#applyBulkAction').on('click', function() {
        ...
        if (action === 'delete') {
            confirmText = 'Are you sure you want to delete ' + ids.length + ' member(s)? This action cannot be undone!';
        }
