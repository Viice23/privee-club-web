    ...
    <div class="container-fluid">
      <div class="page-header">
        <div class="container-fluid d-flex justify-content-between align-items-center">
          <h3><i class="fas fa-user-plus"></i> {{ isset($member) && $member->id ? 'Update Ready Member' : 'Add Ready Member' }}</h3>
          <button class="btn btn-primary">
            <a href="{{ route('admin.ready.members') }}" class="back-btn" style="color: white; text-decoration: none;">← Back</a>
          </button>
        </div>
      </div>
    </div>
        ...
        <div class="tab-pane fade show active" id="male" role="tabpanel" aria-labelledby="male-tab">
          <div class="card mt-3">
            <div class="card-body table-responsive">
              <form action="{{ isset($member) && $member->id ? route('admin.edit.ready.member', base64_encode($member->id)) : route('admin.add.ready.member') }}" method="POST" enctype="multipart/form-data">
                @csrf
                
                <div class="row">
                  <!-- Basic Information -->
                  <div class="col-md-6">
                    <h5 class="mb-3" style="color: #5f42aa; border-bottom: 2px solid #e5e0ff; padding-bottom: 10px;">Basic Information</h5>
                    ...
                    <div class="form-group mb-3">
                      ...
                      @else
                        <div class="mb-2">
                          <img src="{{ asset('assets/images/default-150x150.png') }}" id="profileImagePreview" 
                            style="width: 150px; height: 150px; border-radius: 50%; object-fit: cover; border: 3px solid #e5e0ff; cursor: pointer;"
                            onclick="document.getElementById('profile_image').click()" 
                            title="Click to select image">
                        </div>
                      @endif
                      <input type="file" name="profile_image" id="profile_image" class="form-control" accept="image/*" style="display: none;" onchange="previewProfileImage(this)">
                      <small class="form-text text-muted">Click on image to change. JPG, PNG, GIF (max 2MB)</small>
                      @error('profile_image')
                    ...
                    <div class="form-group mb-3">
                      ...
                      <select name="gender" class="form-control" required>
                        ...
                        <option value="Male" {{ old('gender', (isset($member) && $member->id ? $member->gender : '')) == 'Male' ? 'selected' : '' }}>Male</option>
                        <option value="Female" {{ old('gender', (isset($member) && $member->id ? $member->gender : '')) == 'Female' ? 'selected' : '' }}>Female</option>
                      </select>
                      @error('gender')
                        <span class="text-danger">{{ $message }}</span>
                      @enderror
                  ...
                  <!-- Physical Attributes -->
                  <div class="col-md-6">
                    <h5 class="mb-3" style="color: #5f42aa; border-bottom: 2px solid #e5e0ff; padding-bottom: 10px;">Physical Attributes</h5>
                ...
                <div class="row mt-3">
                  ...
                  <div class="col-md-6">
                    <h5 class="mb-3" style="color: #5f42aa; border-bottom: 2px solid #e5e0ff; padding-bottom: 10px;">Location & Preferences</h5>
                      ...
                      <select name="region" id="region" class="form-control">
                        ...
                        @foreach($regions as $region)
                          <option value="{{ $region->region }}" 
                            ...
                            {{ $region->region }} @if($region->country) ({{ $region->country->country }}) @endif
                          </option>
                        @endforeach
                      </select>
                    </div>
                      ...
                      <select name="city" id="city" class="form-control">
                        ...
                        @foreach($cities as $city)
                          <option value="{{ $city->city }}" 
                            ...
                            {{ $city->city }} @if($city->region) ({{ $city->region->region }}) @endif
                          </option>
                        @endforeach
                      </select>
                    </div>
                  ...
                  <!-- About & Status -->
                  <div class="col-md-6">
                    <h5 class="mb-3" style="color: #5f42aa; border-bottom: 2px solid #e5e0ff; padding-bottom: 10px;">About & Status</h5>
                    
                    <div class="form-group mb-3">
                      <label for="about_me">About Me</label>
                      <textarea name="about_me" class="form-control" rows="4"
                        placeholder="Enter about me">{{ old('about_me', isset($member) && $member->id ? $member->about_me : '') }}</textarea>
                    </div>

                    <div class="form-group mb-3">
                      <label for="about_match">My Perfect Match</label>
                      <textarea name="about_match" class="form-control" rows="4"
                        placeholder="Enter perfect match description">{{ old('about_match', isset($member) && $member->id ? $member->about_match : '') }}</textarea>
                    </div>
                    ...
                    <div class="form-group mb-3">
                      ...
                      <div>
                        <label class="switch">
                          <input type="checkbox" name="is_active" {{ old('is_active', (isset($member) && $member->id && $member->is_active == 1)) ? 'checked' : '' }}>
                    ...
                    <div class="form-group mb-3">
                      ...
                      <div>
                        <label class="switch">
                          <input type="checkbox" name="show_on_mobile" {{ old('show_on_mobile', (isset($member) && $member->id && $member->show_on_mobile == 1)) ? 'checked' : '' }}>
                          <span class="slider"></span>
                        </label>
                ...
                <div class="mt-4">
                  <button type="submit" class="btn btn-success">
                    <i class="fas fa-save"></i> {{ isset($member) && $member->id ? 'Update Ready Member' : 'Add Ready Member' }}
                  </button>
